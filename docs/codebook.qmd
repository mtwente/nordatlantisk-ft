...
cabinets_fo_gl <- read.delim(here("assets", "background_data", "cabinets_fo_gl.csv"), sep = ";", stringsAsFactors = FALSE)
...
`cabinets_fo_gl` is provided as `csv` file in `/assets/background_data/`. It contains information on the coalition governments of Greenland (Naalakersuisut) and the Faroe Islands (Landsstýri), in particular a timeline of cabinets including the parties that are part of each cabinet. As per `r format(file.info(here("assets", "background_data", "cabinets_fo_gl.csv"))$mtime, "%B %Y")`, `cabinets_fo_gl` contains information on `r nrow(cabinets_fo_gl)` cabinets stored in `r ncol(cabinets_fo_gl)` variables. On this list of cabinets are `r sum(cabinets_fo_gl$location == "GL", na.rm = TRUE)` Greenlandic and `r sum(cabinets_fo_gl$location == "FO", na.rm = TRUE)` Faroese cabinets.
...
#### northatlantic_ft\$governing_party_at_home

`governing_party_at_home` is created by using [`join_results()`](/src/targets/join_results.R) on `northatlantic_votes`, `ballot_results_ft`, `MP_names` and `cabinets_fo_gl`, indicating whether the MP is a member of a political party that is part of the governing Naalakkersuisut (GL) resp. Landsstýri (FO) at the time. See [`cabinets_fo_gl`](#cabinets_fo_gl).

```{r}
summary(northatlantic_ft$governing_party_at_home)
```
